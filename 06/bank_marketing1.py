...
from tensorflow.examples.tutorials.mnist.mnist import inference
...
def test_model(test_x, test_y):
    ...
    accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.arg_max(output, axis=1), tf.arg_max(y, axis=1)), ))
    with tf.Session() as sess:
        saver = tf.train.Saver()
        saver.restore(sess,os.path.join("models", "mnist.ckpt"))
